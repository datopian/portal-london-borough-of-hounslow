        ...
        <section className="bg-white rounded-lg lg:p-8 p-4 mb-4 max-h-[400px] overflow-y-auto">
            ...
            .map((group) => (
              <div key={group.id}>
                <Field
                  ...
                  value={group.name}
                ></Field>
                <label className="ml-1.5 text-secondary" htmlFor={group.id}>
                  {group.display_name}
                </label>
              </div>
            ))}
          {groupsData.length > 5 && (
            <button
              ...
              type="button"
              className="bg-gray-300 px-2 rounded text-secondary mt-2"
            >
          ...
          {orgsData.slice(0, seeMoreOrgs ? orgsData.length : 5).map((org) => (
            <div key={org.id}>
              <Field
                ...
                value={org.name}
              ></Field>
              <label className="ml-1.5 text-secondary" htmlFor={org.id}>
                {org.display_name}
              </label>
            </div>
          ))}
